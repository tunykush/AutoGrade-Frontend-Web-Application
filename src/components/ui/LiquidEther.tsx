...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Viscous extends ShaderPass {
      ...
      update(p: any) {
        let fi: any, fo: any; this.uniforms.v.value = p.viscous;
        for (let i = 0; i < p.iterations; i++) { if (i % 2 === 0) { fi = this.props.output0; fo = this.props.output1; } else { fi = this.props.output1; fo = this.props.output0; } this.uniforms.velocity_new.value = fi.texture; this.props.output = fo; this.uniforms.dt.value = p.dt; super.update(); }
        return fo;
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(p: any) {
        let pi: any, po: any;
        for (let i = 0; i < p.iterations; i++) { if (i % 2 === 0) { pi = this.props.output0; po = this.props.output1; } else { pi = this.props.output1; po = this.props.output0; } this.uniforms.pressure.value = pi.texture; this.props.output = po; super.update(); }
        return po;
      }
    }
    ...
    class WebGLManager {
      props: any; output!: Output; autoDriver: AutoDriver; lastUserInteraction: number; running = false;
